...
from features.event_bus import EventBus
...
class TradingBot:
    def __init__(self):
        ...
        self.exchange = BloFinExchange(API_KEY, API_SECRET, API_PASSPHRASE)
                ...
                async with websockets.connect(WS_URL, ping_interval=20, ping_timeout=20) as ws:
                    ...
                    for symbol in self.symbols:
                        ...
                        for tf in TIMEFRAMES:
                            candles = self.exchange.get_candles(symbol, tf, CANDLE_LIMIT)
                            if candles:
                    ...
                    def handle_market_data(message):
                        ...
                        candles = self.exchange.get_candles(symbol, "1m", 1)
                        if candles:
                    ...
                    while True:
                        try:
                            ...
                            if "data" in data and data["data"]:
                                ...
                                self.event_bus.publish("market_data", {"symbol": symbol, "price": price, "volume": volume})
    ...
    async def health_check(self):
        ...
        try:
            if not self.exchange.validate_credentials():
                ...
                return False
            balance = self.exchange.get_account_balance(self)
            if balance is None:
    ...
    async def main(self):
        ...
        self.account_balance = self.exchange.get_account_balance(self)
        ...
        for symbol in self.symbols:
            price_volume = self.exchange.get_price(symbol)
            if not price_volume:
